...
async def initialize_runtime_config(app: FastAPI):
    ...
    try:
        app.state.ef = get_ef(
            rag_config.get('rag.embedding_engine'),
            rag_config.get('rag.embedding_model'),
        )
    except Exception as e:
        log.warning(f'Embedding engine disabled: {e}')
        app.state.ef = None
    ...
    if (
        ...
        try:
            ...
            log.warning(f'Reranking disabled: {e}')
            app.state.rf = None
    else:
        app.state.rf = None

except Exception as e:
    log.warning(f'RAG initialization skipped: {e}')
    app.state.ef = None
    app.state.rf = None
